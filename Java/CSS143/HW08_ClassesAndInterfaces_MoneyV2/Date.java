...
public class Date implements Serializable, Comparable, Cloneable 
{
    ...
    //Other Methods
    /**
     *  Pre-Condition: the object being compared in isEqual() is of the class Date
    ...
    public boolean isEqual(Object toCompare){
        try{
            if(toCompare.getClass()!=(new Date()).getClass()){ //Makes sure object is a Date object
                ...
                Date that=new Date((Date)toCompare);
    ...
    //**Implemented Interfeces Overrides**//
    /**
     * Method: clone()
     * Returns a copy of this Date to whereever it is being called.
     */
    public Date clone(){
        try{
            Date copy= (Date)super.clone();
            copy.day=(int)day;
            copy.month=(int)month;
            copy.year=(int)year;
            return copy;
        }
     ...
     * Pre-condition: the object to be compared must also be of the class Date
    ...
    public int compareTo(Object compare){
        try{
            if(compare.getClass()!=(new Date()).getClass()) //Makes sure comparing same class 
                    throw new DateException("Trying to compare different entities");
            Date that=(Date)compare;
            if(isEqual(that))
                return 0;  
                ...
                if(this.month<that.month)
                    ...
                    return 1;
                else{//The months are equal
                    if(this.day<that.day)
                        return -1;
                    else if(this.day>that.day)
                        return 1;
                    else
                        return 0; //the two dates are equal(should never get here)
                }
            }
        }
        catch(DateException e){
