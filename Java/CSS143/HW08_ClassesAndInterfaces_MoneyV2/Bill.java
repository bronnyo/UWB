 ...
 */
public class Bill implements Serializable, Comparable, Cloneable
{
     ...
     *                 definition isEqual() within their own classes.
    ...
    public boolean isEqual(Object o){
        try{
            if(o.getClass()!=(new Bill()).getClass()){ //Makes sure comparing same class 
                ...
                Bill that=(Bill) o;
                if(this.amount.isEqual(that.amount)&&this.dueDate.isEqual(that.dueDate)
                    && this.originator.equals(that.originator)){
                    if(this.paidDate==null && that.paidDate==null)
                        return true;  
                    else if((this.paidDate!=null && that.paidDate!=null)&&this.dueDate.isEqual(that.dueDate))
                        return true;
                }
            }
        }
    ...
    /**
     * Method: clone()
     * Returns a copy of this Bill to whereever it is being called.
     */
    public Bill clone(){
        try{
            Bill copy= (Bill)super.clone();
            copy.amount=(Money)amount.clone(); 
            copy.dueDate=(Date)dueDate.clone();
            if(paidDate!=null)
                copy.paidDate=(Date)paidDate.clone();
            else
                copy.paidDate=null;
            copy.originator=(String)originator;
            return copy;
        }
     ...
     * Pre-condition: the object to be compared must also be of the class Bill
     ...
     * Heiarchy: Paid Date --> Due Date --> Amount (Originator is excluded)
     */
    public int compareTo(Object compare){
        try{
            if(compare.getClass()!=(new Bill()).getClass()) //Makes sure comparing same class 
                    throw new BillException("Trying to compare different entities");
            
            Bill that=new Bill((Bill)compare);
            
            if(isEqual(that))
                return 0;
            else if(this.paidDate!= null && that.paidDate!=null)           
                return this.paidDate.compareTo(that.paidDate);
            else if(this.paidDate!= null && that.paidDate==null)
                return -1;
            else if(this.paidDate== null && that.paidDate!=null)
                return 1;
